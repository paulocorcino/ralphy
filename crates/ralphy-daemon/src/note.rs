...
";
    ...
    /// Containers built outside Rust: Python's `zlib` (raw deflate, `wbits=-15`)
    /// at level 9 (a Huffman block) and level 0 (a STORED block), then masked.
    /// Notes stay on disk and in git, so a note written by an earlier build must
    /// read back whichever deflate backend decodes it (#443).
    #[test]
    fn a_container_from_another_deflate_encoder_reads_back() {
        let text = "# Groceries\n\n- [ ] celery\n- [ ] celery again, celery again\n";
        let huffman: &[u8] = &[
            0x52, 0x4e, 0x4f, 0x54, 0x01, 0x3b, 0x00, 0x00, 0x00, 0x01, 0x18, 0x3f, 0x7b, 0x59,
            0x15, 0x1f, 0x32, 0x98, 0x02, 0x62, 0xb2, 0x71, 0x88, 0x04, 0x97, 0x04, 0xc6, 0x1a,
            0x1c, 0xdd, 0x97, 0x18, 0x32, 0xf8, 0x0a, 0xae, 0x7c, 0xb7, 0xfc, 0x76, 0x79, 0xb4,
            0xa7, 0xef, 0xa4, 0x2b, 0x5a,
        ];
        let stored: &[u8] = &[
            0x52, 0x4e, 0x4f, 0x54, 0x01, 0x3b, 0x00, 0x00, 0x00, 0x53, 0x75, 0x4f, 0x90, 0x6c,
            0x79, 0x71, 0x58, 0x20, 0x21, 0x2c, 0x31, 0xe1, 0x33, 0x34, 0x6c, 0x58, 0x44, 0x62,
            0x74, 0xc8, 0x7a, 0x0c, 0x3f, 0x31, 0x2b, 0x23, 0x31, 0xe1, 0x23, 0x5b, 0x32, 0x72,
            0x15, 0x6f, 0x09, 0xb3, 0x39, 0x34, 0x73, 0x37, 0x3c, 0x36, 0x74, 0xf2, 0x3d, 0x30,
            0x76, 0x3c, 0x62, 0x6f, 0x37, 0xf6, 0x36, 0x34, 0x6d, 0x2b, 0x6e, 0x2e, 0x33, 0xf2,
            0x33, 0x3f, 0x15,
        ];
        assert_eq!(decode(huffman).unwrap(), text);
