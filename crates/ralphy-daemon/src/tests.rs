    ...
    let record = interactive
        ...
        .find(|r| {
            r.get("agent").and_then(|v| v.as_str()) == Some("gemini")
                && r.get("session_id").and_then(|v| v.as_str()) == Some("ralphy-probe-p1p2p3p4p6")
        })
...
fn a_notes_hand_and_size_are_a_closed_set_on_both_sides() {
    ...
    assert!(
        notes.contains("el._noteEditor?.redrawDiagrams?.()") && crepe.contains("redrawDiagrams:"),
...
fn the_narrow_pane_criterion_is_the_panes_own_width() {
    ...
    for (pane, name) in [("\n.viewer {", "viewer"), ("\n.kanban {", "kanban")] {
        ...
        assert!(
            block.contains("container-type: inline-size")
                && block.contains(&format!("container-name: {name}")),
...
fn shell_lists_the_fences() {
    ...
    for (jump, fold, focus) in [
        ...
        assert!(
            !b.contains("anchorIntoView(restoreRect") && !b.contains("bringIntoView(restoreRect"),
...
fn the_design_system_scrollbar_is_the_default_not_a_list() {
    ...
    assert!(
        squeezed
            .contains("* { scrollbar-width: thin; scrollbar-color: var(--border) transparent; }"),
...
fn the_board_surfaces_the_plan_the_next_run_would_execute() {
    ...
    assert!(
        app_squeezed.contains(r#"const ok = await this.askConfirm({ title: "Discard this plan?","#),
