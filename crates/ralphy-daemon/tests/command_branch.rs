//! The branch-list Query verb reaches the child and answers on the requesting id
//! (issue #199; ADR-0036 §2): a `branch.list` for a registered repo spawns-and-
//! COLLECTS `command_test_child` (pointed at via `RALPHY_EXE_OVERRIDE`), which
//! echoes its argv — proving `branch_list_argv` → CLI end to end. The reply carries
//! `status:"ok"`; because the child's echo is not valid JSON, the `branches` field
//! is the raw collected stdout, which must contain `branch list --format json`.
//!
...
#[tokio::test]
async fn branch_list_argv_reaches_the_child() {
    let dir = tempfile::tempdir().unwrap();
    let registry_path = dir.path().join("repos.toml");
    let mut store = registry::RegistryStore::default();
    let slug = "owner/branches";
    ...
    assert_eq!(reply["status"], "ok", "branch query is ok; got {reply}");
    // The child's echo is not JSON, so `branches` is the raw collected stdout — it
    // must carry the composed argv (proving `branch_list_argv` reached the child).
    let raw = reply["branches"].as_str().expect("raw branches string");
    assert!(
        raw.contains("branch list --format json"),
        "the branch-list argv must reach the child; got: {raw:?}"
    );
}
