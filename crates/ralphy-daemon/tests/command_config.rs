//! The config Query verb reaches the child and answers on the requesting id
//! (issue #195; ADR-0036 §2): a `config.get` for a registered repo spawns-and-
//! COLLECTS `command_test_child` (pointed at via `RALPHY_EXE_OVERRIDE`), which
//! echoes its argv — proving `config_argv` → CLI end to end. The reply carries
//! `status:"ok"`; because the child's echo is not valid JSON, the `config` field
//! is the raw collected stdout, which must contain `config get --json`.
//!
...
#[tokio::test]
async fn config_get_argv_reaches_the_child() {
    ...
    assert_eq!(reply["status"], "ok", "config query is ok; got {reply}");
    // The child's echo is not JSON, so `config` is the raw collected stdout — it
    // must carry the composed argv (proving `config_argv` reached the child).
    let raw = reply["config"].as_str().expect("raw config string");
    assert!(
        raw.contains("config get --json"),
        "the config argv must reach the child; got: {raw:?}"
    );
}
