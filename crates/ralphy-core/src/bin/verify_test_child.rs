...
//!
//! - `exit-leaking-grandchild` — spawn a copy of itself (in `sleep` mode) that
//!   inherits this process's stdout pipe, then exit 0 *immediately*. This is the
//!   FinCal #29 shape (#156): the foreground command exits clean in moments, but
//!   the orphaned grandchild keeps the stdout write-end open, so an unbounded
//!   output drain would block forever even though the exit status is already in
//!   hand. The gate must return within its collect grace and report exit 0.
