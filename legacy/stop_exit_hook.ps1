# stop_exit_hook.ps1 — Stop hook for the Ralphy execution session.
#
...
# the agent's exit signal to $env:RALPHY_FLAG_FILE. The orchestrator is polling
# that file and owns the actual process termination (it holds the PID).
#
# No-op unless RALPHY_FLAG_FILE is set, so this hook is harmless if it ever
# leaks into a normal interactive session.

$ErrorActionPreference = 'SilentlyContinue'

$flag = $env:RALPHY_FLAG_FILE
if (-not $flag) { exit 0 }
...
if ($msg -match 'RALPHY_DONE_EXIT') {
    'DONE' | Set-Content -LiteralPath $flag -Encoding utf8
    exit 0
}
$m = [regex]::Match($msg, 'RALPHY_BLOCKED_EXIT\s*(.*)')
if ($m.Success) {
