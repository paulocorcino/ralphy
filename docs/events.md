...
| `run.stopped` | The operator asked the run to stop (`ralphy stop`, ADR-0054) — fired at the MOMENT of the stop, ahead of the teardown the matching `run.finished` (`outcome: stopped`) reports | `number` — the issue in flight, or `null` when the stop landed between issues; `subject` names that issue when there is one |
...
| `run.finished` | Clean end of a run (never on crash/kill — detect those by heartbeat silence) | `outcome` (`completed` \| `non_green` \| `deadline` \| `stop_before` \| `stopped` \| `no_work`; only `completed` means the whole queue was worked; `stopped` is the operator's own `ralphy stop`, ADR-0054), `issues_done`, `issues_skipped`, `issues_total`, `issues_blocked`, `issues_hitl`, `issues[]` — per-issue rollup `{number, title, status, kind?, blocked_by?, review_only?}` (`status`: `done` \| `skipped` \| `blocked` \| `infeasible` \| `needs_split` \| `non_green` \| `hitl` \| `planned` (a plan-only pass superseded by the next issue — a dry run); `kind` and `blocked_by[]` only on a `skipped` — `blocked_by` names the open blocker(s), empty for non-dependency skips); `review_only` is the count of the issue's acceptance-ledger lines that closed `[review-only]`, present only when non-zero — attention debt on a *delivered* issue, never unfinished work; `tokens_total {up,cr,cw,out}`, `duration_s`. Carries **no** `data.agent` block. The `issues[]` rollup and the five scalar counts are derived from the **same** source — the run's own closing summary, the one the console's final panel prints — so they can never disagree; only a legacy emission that carried no rollup falls back to the folded run state to build `issues[]`. The rollup lists only issues that **entered the run**; key completeness off the scalar `issues_total`, not the array length. In particular an issue that halted the run via `stop-before` never enters `issues[]` (the run stops ahead of it, so it is never worked) and its number is **not** on this event — `outcome: stop_before` says only that the run halted that way; read the halting issue's number off `queue.built`'s `stop_before`. `no_work` (#222) is the empty-queue border: the run emits the full triad (`queue.built` with `count: 0` → `run.started` → `run.finished`) with every count at `0` and no `issues[]` entries |
