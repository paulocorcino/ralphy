...
daemon can call**. `program_name` resolves a bare name against `PATH`, so such a
variant compiles, passes review, and then fails to launch. ADR-0032 §10 bars the
daemon from importing the core, hence from importing the adapter crate — put the
