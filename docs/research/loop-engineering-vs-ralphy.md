...
| **Token blowout** | Hard caps before shipping | **Time** caps, not token caps (no API spend to cap): `--max-minutes-per-issue` (default ~60 min; pass `0` to opt out), `--deadline-hours`, stop-at-first-failure, bounded repair. Spend is *measured* (`usage.jsonl`), not budgeted. |
