...
3. Write `.ralphy/plan.md` with this exact shape:
   ...
   - [ ] Self-review: run the **inline `reviewer` skill** (auto-discovered from
         the `--skills-dir` skills store at `.ralphy/skills/reviewer/`, loaded on
         demand via the native `skill` tool), invoked by name over ONLY the
         commits you made for this issue — **not** a subagent, and not the whole
         branch; for a small mechanical diff, write this step as a direct
         adversarial re-read of the diff instead (see the self-review rule below).
...
- The penultimate step is a self-review over ONLY the commits you made for
  ...
  - changes with real domain logic or a multi-file/multi-crate surface get the
    ...
    the `--skills-dir` skills store at `.ralphy/skills/reviewer/`, loaded on
    demand via the native `skill` tool), invoked by name — **not** a subagent,
    and not the whole branch;
